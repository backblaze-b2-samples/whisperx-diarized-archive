...
// Surfaces every common local setup gotcha *before* uvicorn or next try
// to start, with actionable error messages.
//
...
// and PLACEHOLDER_VALUES. B2_REGION has a safe default + derived endpoint,
// so it isn't treated as a hard requirement here.
...
function checkEnv() {
  ...
  if (placeholders.length > 0) {
    fail(
      ...
      "Edit .env and replace placeholders with your real B2 credentials (https://secure.backblaze.com/app_keys.htm?utm_source=github&utm_medium=referral&utm_campaign=ai_artifacts&utm_content=whisperx-diarized-archive)",
    );
  }
}

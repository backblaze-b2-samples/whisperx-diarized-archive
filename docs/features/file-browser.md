...
## Purpose
List, preview, download, and delete **everything** in the Backblaze B2
bucket — source `media/`, plus the `transcripts/`, `segments/`, and
`embeddings/` artifacts the pipeline writes. This is the full-bucket view;
the scoped, media-only view is the [Archive Library](archive-library.md).
...
- [Archive Library](archive-library.md) — the media-only scoped view
